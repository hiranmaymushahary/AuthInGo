...
func NewRoleRouter(_roleController *controllers.RoleController, _userRoleRepo repo.UserRoleRepository) Router {
    return &RoleRouter{
...
func (rr *RoleRouter) Register(r chi.Router) {
    ...
    r.With(middlewares.JWTAuthMiddleware, middlewares.RequireAllRoles(rr.userRoleRepo, "admin")).Post("/roles/{userId}/assign/{roleId}", rr.roleController.AssignRoleToUser)
}